...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function DockRail({ t, tabEntries, openDetails }) {
			...
			if (open) return null;
			// Mirrored (ext) panels are not listed in the collapsed rail: they
			// are strip-only, and a stray mirror (e.g. a shell panel that got
			// mirrored before an update) must never surface as a rail tile.
			const tiles = entries
				.filter((entry) => !String(entry.options?.key ?? "").startsWith("ext:"))
				.map((entry) => {
				const key = entry.options?.key;
				const label = entry.options?.label || key;
