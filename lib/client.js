	...
	factory: (require) => {
		...
		function TabsContainer(props) {
			...
			const refresh = react.useCallback(() => {
				...
				const autoKeys = keys.filter((k) => !k.startsWith("ext:"));
				setEntries((prev) => {
			...
			// (blue) chip to hide its panel, a dimmed chip to show it. ──
		...
		function mountThirdPartyMirror(ctx, selfComponent) {
			 ...
			 * locale NS "conversation", declaring the conversation.details.tool
			 * child slot). It is the shell's NATIVE details content, not a
			 * third-party panel — mirroring it made a stray "Details"/"De"
			 * entry appear in the rail/chips/layout. Both signals together are
			 * specific enough that no third-party entry is at risk.
			 */
			const isShellDetailsPanel = (entry) => {
				const opts = entry.options ?? {};
				const children = opts.children && typeof opts.children === "object" ? opts.children : {};
				const declaresToolSlot = Object.keys(children).some((k) => k.includes("conversation.details"));
				return opts.locale === "conversation" && declaresToolSlot;
			};
			const sync = () => {
