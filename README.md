...
| **面板条 (strip)** | 顶部列出所有已注册面板：亮=已打开、暗=已关闭；**点击切换显示/隐藏**（点 × 关闭后保持隐藏，不会 2 秒后自动恢复；点暗色 chip 才重新显示）；**chip 可拖拽排序**（持久化）；右侧 `↺` 重置布局、`»` 收起侧栏 |
| **Blender 风格 DockRail** | 侧栏收起时，右侧边缘竖向显示**当前打开**面板的短标签（关闭的面板不显示磁贴，与实际状态一致；点击展开对应面板，重新显示走面板条 chip）；无打开面板时显示 `≡` 兜底展开按钮 |
